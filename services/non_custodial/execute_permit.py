...
async def execute_permit(permit_request: ExecutePermitRequest):
    ...
    try:
        ...
        if result["allowance"] >= permit_request.value:
            logger.info(f"Skip permit transaction due to it has allowance.")
            return {
                "success": True,
                "txHash": "",
                "status": "",
                "message": "Skip permit transaction due to it has allowance",
...
async def transfer_from(req: TransferFromRequest):
    ...
    try:
        ...
        try:
            ...
            if error_code:
                raise Exception(f"[{error_code.code}] transferFrom simulation failed: {error_msg}")
            else:
        ...
        if result.get("success"):
            ...
            for _ in range(max_attempts):
                try:
                    poll = await handler.get_transaction_status(tx_hash)
                    logger.info(f"========= poll from transaction status: {poll} =========")
                    if poll.get("success") and poll.get("status") == "confirmed":
                        return {
                            "success": True,
                            "txHash": tx_hash,
