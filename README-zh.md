...
**[`host_agent`](https://github.com/Zen7-Labs/Zen7-Payment-Agent/tree/main/host_agent)** 目录为多代理协同架构的核心实现，主代理（`host agent`）使用 `gemini-2.0-flash-lite` 模型作为核心协调器，负责查询理解、状态管理和响应协调。子代理系统（`sub_agents/`）包含五个专门的代理：`payer_agent` 负责支付方的订单创建、EIP-712签名生成和钱包余额验证；`settlement_agent` 专注于结算流程，确认支付详情、执行链上交易并监控交易状态；`payee_agent` 则处理收款方相关业务，接收结算通知、确认订单创建并通知支付完成；`order_agent` 负责订单处理和意图识别，自动路由到不同代理；`allowance_agent` 提供授权额度查询功能，支持多链代币授权查询。
...
**[`dao`](https://github.com/Zen7-Labs/Zen7-Payment-Agent/tree/main/dao)** - 数据访问层实现，集成 PostgreSQL + SQLModel 实现数据持久化。包含数据库模型定义（`model.py`）、数据库连接管理（`database.py`）和数据访问接口（`app.py`），支持订单、支付、结算、意图和审计事件等完整的业务数据管理。

**[`task_manager`](https://github.com/Zen7-Labs/Zen7-Payment-Agent/tree/main/task_manager)** - 任务管理层实现，采用工厂模式设计。`payment_service.py` 提供支付服务的统一接口，`task_scoped_manager.py` 实现任务作用域管理，确保不同支付任务的隔离和生命周期管理。

**[`services`](https://github.com/Zen7-Labs/Zen7-Payment-Agent/tree/main/services)** - 完整的区块链服务实现。签名服务支持 EVM 链（`execute_sign.py`）和 Solana 链（`execute_sign_solana.py`）的签名功能，EVM 提供 EIP-712 类型化数据签名，支持 USDC 和 DAI 的许可签名；转账处理器采用基类抽象设计（`base_handler.py`），分为 `custodial/` 托管模式（后端管理钱包以简化用户体验）和 `non_custodial/` 非托管模式（用户控制私钥以增强安全性），支持 EVM（`evm_transfer_handler.py`）和 Solana（`solana_transfer_handler.py`）两种区块链；数据服务层包含意图记录（`intent.py`）、审计事件（`audit_event.py`）、结算批次（`settlement_batch.py`）和结算明细（`settlement_detail.py`），实现交易全流程追踪；常量配置（`constants.py`）集中管理区块链网络配置、合约地址和链 ID；许可执行（`execute_permit.py`）负责 ERC-20 代币的授权和许可执行。
...
这种设计为开发者提供了灵活的测试环境，支持多链、多币种的完整支付解决方案，同时确保了与主流钱包和区块链网络的良好兼容性。
